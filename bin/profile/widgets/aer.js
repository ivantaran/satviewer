...
//	canvas.close();
	canvas.setSize(128, 82);
	canvas.setPos(parentWidth - 128, 0);
	canvas.fill(0x0);
	canvas.setPen(0xFFFFFF);
	canvas.setFont("Arial", 10, 75, false);
	canvas.drawText(8, 18, "AZM " + toDegMin(aerv[0]));
	canvas.drawText(8, 36, "ELV " + toDegMin(aerv[1]));
	canvas.drawText(8, 54, "RNG " + toKm(aerv[2]));
	canvas.drawText(8, 72, "VEL " + toKmSec(aerv[3]));
}
