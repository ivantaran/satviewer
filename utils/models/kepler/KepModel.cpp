...
int KepModel::modelInit(int whichconst, double _jdsatepoch, double _bstar, double _inclo,
                        double _argpo, double _ecco, double _nodeo, double _mo, double _no) {
    ecco = _ecco;
    argpo = _argpo;
    inclo = _inclo;
    mo = _mo;
...
int KepModel::model(double tsince) {
    ...
    this->v[0] = Vr * (cos(omg) * cos(u) - sin(omg) * sin(u) * cos(inclo)) -
                 Vv * (cos(omg) * sin(u) + sin(omg) * cos(u) * cos(inclo));
    this->v[1] = Vr * (sin(omg) * cos(u) + cos(omg) * sin(u) * cos(inclo)) -
                 Vv * (sin(omg) * sin(u) - cos(omg) * cos(u) * cos(inclo));
    this->v[2] = Vr * sin(u) * sin(inclo) + Vv * cos(u) * sin(inclo);
    Satellite::getGeod();
    return 0;
} // end kepler

double KepModel::gstime(double jdut1) {
