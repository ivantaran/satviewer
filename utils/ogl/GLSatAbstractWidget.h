...
class GLSatAbstractWidget : public QOpenGLWidget,
                            protected QOpenGLFunctions_2_0 { // TODO QOpenGLWidget - to protected
    Q_OBJECT
public:
    //    ZrvIoList ioList;
    ...
    virtual void setIcon(Satellite *sat, const QString &fileName = "") {
        Q_UNUSED(sat);
        Q_UNUSED(fileName);
    }
    virtual void setIcon(Location *loc, const QString &fileName = "") {
        Q_UNUSED(loc);
        Q_UNUSED(fileName);
    }
    ...
    uint32_t colorNet() const {
        return m_colorNet;
    }
    uint32_t colorNetFont() const {
        return clrNetFont;
    }
    uint32_t colorNight() const {
        return clrNight;
    }
