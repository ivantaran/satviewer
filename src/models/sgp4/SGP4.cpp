...
static void initl(
    ...
    // gravconsttype whichconst,
    double xke, double j2, double ecco, double epoch, double inclo, double &no, char &method,
    double &ainv, double &ao, double &con41, double &con42, double &cosio, double &cosio2,
    double &eccsq, double &omeosq, double &posq, double &rp, double &rteosq, double &sinio,
...
static void initl(
    ...
    // gravconsttype whichconst,
    double xke, double j2, double ecco, double epoch, double inclo, double no_kozai, char opsmode,
    char &method, double &ainv, double &ao, double &con41, double &con42, double &cosio,
    double &cosio2, double &eccsq, double &omeosq, double &posq, double &rp, double &rteosq,
    double &sinio, double &gsto, double &no_unkozai) {
    /* --------------------- local variables ------------------------ */
...
bool sgp4init(gravconsttype whichconst, char opsmode, const int satn, const double epoch,
              ...
              const double xargpo, const double xinclo, const double xmo, const double xno_kozai,
              const double xnodeo, elsetrec &satrec) {
    /* --------------------- local variables ------------------------ */
    double ao, ainv, con42, cosio, sinio, cosio2, eccsq, omeosq, posq, rp, rteosq, cnodm, snodm,
        cosim, sinim, cosomm, sinomm, cc1sq, cc2, cc3, coef, coef1, cosio4, day, dndt, em, emsq,
        eeta, etasq, gam, argpm, nodem, inclm, mm, nm, perige, pinvsq, psisq, qzms24, rtemsq, s1,
        s2, s3, s4, s5, s6, s7, sfour, ss1, ss2, ss3, ss4, ss5, ss6, ss7, sz1, sz2, sz3, sz11, sz12,
        sz13, sz21, sz22, sz23, sz31, sz32, sz33, tc, temp, temp1, temp2, temp3, tsi, xpidot,
        xhdot1, z1, z2, z3, z11, z12, z13, z21, z22, z23, z31, z32, z33, qzms2t, ss, x2o3, r[3],
        v[3], delmotemp, qzms2ttemp, qzms24temp;
    ...
    getgravconst(whichconst, satrec.tumin, satrec.mu, satrec.radiusearthkm, satrec.xke, satrec.j2,
    ...
    initl(satrec.xke, satrec.j2, satrec.ecco, epoch, satrec.inclo, satrec.no_kozai,
          satrec.operationmode, satrec.method, ainv, ao, satrec.con41, con42, cosio, cosio2, eccsq,
          omeosq, posq, rp, rteosq, sinio, satrec.gsto, satrec.no_unkozai);
        ...
        if (satrec.isimp != 1) {
            ...
            satrec.t5cof =
                0.2 * (3.0 * satrec.d4 + 12.0 * satrec.cc1 * satrec.d3 +
                       6.0 * satrec.d2 * satrec.d2 + 15.0 * cc1sq * (2.0 * satrec.d2 + cc1sq));
        }
    } // if omeosq = 0 ...
...
bool sgp4(elsetrec &satrec, double tsince, double r[3], double v[3]) {
    ...
    if (satrec.method == 'd') {
        ...
        dspace(satrec.irez, satrec.d2201, satrec.d2211, satrec.d3210, satrec.d3222, satrec.d4410,
               ...
               satrec.domdt, satrec.argpo, satrec.argpdot, satrec.t, tc, satrec.gsto, satrec.xfact,
               satrec.xlamo, satrec.no_unkozai, satrec.atime, em, argpm, inclm, satrec.xli, mm,
    ...
    if (satrec.method == 'd') {
        dpper(satrec.e3, satrec.ee2, satrec.peo, satrec.pgho, satrec.pho, satrec.pinco, satrec.plo,
              ...
              satrec.xl3, satrec.xl4, satrec.zmol, satrec.zmos, satrec.inclo, 'n', ep, xincp, nodep,
...
void getgravconst(gravconsttype whichconst, double &tumin, double &mu, double &radiusearthkm,
...
void twoline2rv(char longstr1[130], char longstr2[130], char typerun, char typeinput, char opsmode,
    ...
    sscanf_s(longstr1, "%2d %5ld %1c %10s %2d %12lf %11lf %7lf %2d %7lf %2d %2d %6ld ", &cardnumb,
             &satrec.satnum, &satrec.classification, sizeof(char), &satrec.intldesg,
             11 * sizeof(char), &satrec.epochyr, &satrec.epochdays, &satrec.ndot, &satrec.nddot,
             &nexp, &satrec.bstar, &ibexp, &satrec.ephtype, &satrec.elnum);
#else
    sscanf(longstr1, "%2d %5ld %1c %10s %2d %12lf %11lf %7lf %2d %7lf %2d %2d %6ld ", &cardnumb,
           &satrec.satnum, &satrec.classification, &satrec.intldesg, &satrec.epochyr,
           &satrec.epochdays, &satrec.ndot, &satrec.nddot, &nexp, &satrec.bstar, &ibexp,
           &satrec.ephtype, &satrec.elnum);
#endif
    if (typerun == 'v') // run for specified times from the file
    {
        if (longstr2[52] == ' ') {
            ...
            sscanf_s(longstr2, "%2d %5ld %9lf %9lf %8lf %9lf %9lf %10lf %6ld %lf %lf %lf \n",
                     ...
                     &satrec.argpo, &satrec.mo, &satrec.no_kozai, &satrec.revnum, &startmfe,
                     &stopmfe, &deltamin);
#else
            sscanf(longstr2, "%2d %5ld %9lf %9lf %8lf %9lf %9lf %10lf %6ld %lf %lf %lf \n",
                   &cardnumb, &satrec.satnum, &satrec.inclo, &satrec.nodeo, &satrec.ecco,
                   &satrec.argpo, &satrec.mo, &satrec.no_kozai, &satrec.revnum, &startmfe, &stopmfe,
                   &deltamin);
#endif
        } else {
#ifdef _MSC_VER
    ...
    } else // simply run -1 day to +1 day or user input times
    {
        if (longstr2[52] == ' ') {
            ...
            sscanf_s(longstr2, "%2d %5ld %9lf %9lf %8lf %9lf %9lf %10lf %6ld \n", &cardnumb,
                     &satrec.satnum, &satrec.inclo, &satrec.nodeo, &satrec.ecco, &satrec.argpo,
                     &satrec.mo, &satrec.no_kozai, &satrec.revnum);
#else
    ...
    if ((typerun != 'v') && (typerun != 'c')) {
        ...
        if (typeinput == 'e') {
...
#ifdef _MSC_VER
            scanf_s("%i %i %i %i %i %lf", &startyear, &startmon, &startday, &starthr, &startmin,
                    &startsec);
#else
            scanf("%i %i %i %i %i %lf", &startyear, &startmon, &startday, &starthr, &startmin,
                  &startsec);
#endif
            fflush(stdin);
...
#ifdef _MSC_VER
            scanf_s("%i %i %i %i %i %lf", &stopyear, &stopmon, &stopday, &stophr, &stopmin,
                    &stopsec);
#else
            ...
            stopmfe =
                (jdstop - satrec.jdsatepoch) * 1440.0 + (jdstopF - satrec.jdsatepochF) * 1440.0;
    ...
    sgp4init(whichconst, opsmode, satrec.satnum,
             (satrec.jdsatepoch + satrec.jdsatepochF) - 2433281.5, satrec.bstar, satrec.ndot,
             satrec.nddot, satrec.ecco, satrec.argpo, satrec.inclo, satrec.mo, satrec.no_kozai,
             satrec.nodeo, satrec);
} // twoline2rv
...
void rv2coe(double r[3], double v[3], const double mu, double &p, double &a, double &ecc,
...
void invjday(double jd, double jdFrac, int &year, int &mon, int &day, int &hr, int &minute,
             double &sec) {
    int leapyrs;
    double days, tu, temp;
